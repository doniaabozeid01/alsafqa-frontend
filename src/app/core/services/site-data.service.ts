import { Injectable } from '@angular/core';
import { DomSanitizer } from '@angular/platform-browser';
import {
  AboutPageContent,
  Brand,
  BrandCard,
  ContactInfo,
  GalleryImage,
  JourneyStep,
  NavLink,
  NewsItem,
  PageHero,
  ProductCategory,
  ServiceItem,
  StatItem,
  StepItem,
} from '../models/site.models';

/**
 * Central place for every piece of static content on the site.
 * When the backend is ready, swap these getters with HTTP calls
 * without touching any component.
 */
@Injectable({ providedIn: 'root' })
export class SiteDataService {
  constructor(private sanitizer: DomSanitizer) {}

  private icon(svgPath: string): any {
    return this.sanitizer.bypassSecurityTrustHtml(
      `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
        stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">
        ${svgPath}
      </svg>`
    );
  }

  getNavLinks(): NavLink[] {
    return [
      { label: 'الرئيسية', path: '/', exact: true },
      { label: 'من نحن', path: '/about' },
      { label: 'المنتجات', path: '/products' },
      { label: 'علامتك التجارية', path: '/', fragment: 'brand' },
      { label: 'معرض', path: '/gallery' },
      { label: 'تواصل معنا', path: '/contact' },
    ];
  }

  getContactInfo(): ContactInfo {
    return {
      phone: '+20 123 456 7890',
      email: 'info@alsafqa.com',
      address: 'القاهرة، مصر',
    };
  }

  getServices(): ServiceItem[] {
    return [
      {
        title: 'توريد المنتجات',
        description: 'توريد منتجات عالية الجودة حسب الطلب',
        icon: this.icon(
          `<path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="m3.3 7 8.7 5 8.7-5"/><path d="M12 22V12"/>`
        ),
      },
      {
        title: 'الاستيراد والتصدير',
        description: 'خبرة واسعة في مختلف الأسواق العالمية',
        icon: this.icon(
          `<circle cx="12" cy="12" r="10"/><path d="M2 12h20"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/>`
        ),
      },
      {
        title: 'الشحن الدولي',
        description: 'شحن بحري وجوي وبري سريع وآمن',
        icon: this.icon(
          `<path d="M17.8 19.2 16 11l3.5-3.5C21 6 21.5 4 21 3c-1-.5-3 0-4.5 1.5L13 8 4.8 6.2c-.5-.1-.9.1-1.1.5l-.3.5c-.2.5-.1 1 .3 1.3L9 12l-2 3H4l-1 1 3 2 2 3 1-1v-3l3-2 3.5 5.3c.3.4.8.5 1.3.3l.5-.2c.4-.3.6-.7.5-1.2z"/>`
        ),
      },
      {
        title: 'التخزين والتوزيع',
        description: 'مستودعات حديثة وخدمات توزيع متكاملة',
        icon: this.icon(
          `<path d="M22 8.35V20a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V8.35A2 2 0 0 1 3.26 6.5l8-3.2a2 2 0 0 1 1.48 0l8 3.2A2 2 0 0 1 22 8.35Z"/><path d="M6 18h12"/><path d="M6 14h12"/><rect x="6" y="10" width="12" height="12"/>`
        ),
      },
      {
        title: 'التخليص الجمركي',
        description: 'تخليص سريع وآمن لجميع أنواع البضائع',
        icon: this.icon(
          `<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="m9 15 2 2 4-4"/>`
        ),
      },
    ];
  }

  getJourneySteps(): JourneyStep[] {
    return [
      {
        order: '01',
        title: 'تقديم الطلب',
        description: 'تواصل معنا وقدّم تفاصيل احتياجاتك',
        icon: this.icon(
          `<path d="M9 12h6m-6 4h6m2 5H7a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5.586a1 1 0 0 1 .707.293l5.414 5.414a1 1 0 0 1 .293.707V19a2 2 0 0 1-2 2z"/>`
        ),
      },
      {
        order: '02',
        title: 'دراسة المشروع',
        description: 'تحليل السوق وتقييم الجدوى والتكلفة',
        icon: this.icon(
          `<path d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5.586a1 1 0 0 1 .707.293l5.414 5.414a1 1 0 0 1 .293.707V19a2 2 0 0 1-2 2z"/>`
        ),
      },
      {
        order: '03',
        title: 'تجهيز الطلب',
        description: 'تجهيز وفحص البضائع وفق معايير الجودة',
        icon: this.icon(
          `<path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="m3.3 7 8.7 5 8.7-5"/><path d="M12 22V12"/>`
        ),
      },
      {
        order: '04',
        title: 'الشحن',
        description: 'شحن آمن وسريع بأفضل وسائل النقل',
        icon: this.icon(
          `<path d="M17.8 19.2 16 11l3.5-3.5C21 6 21.5 4 21 3c-1-.5-3 0-4.5 1.5L13 8 4.8 6.2c-.5-.1-.9.1-1.1.5l-.3.5c-.2.5-.1 1 .3 1.3L9 12l-2 3H4l-1 1 3 2 2 3 1-1v-3l3-2 3.5 5.3c.3.4.8.5 1.3.3l.5-.2c.4-.3.6-.7.5-1.2z"/>`
        ),
      },
      {
        order: '05',
        title: 'التخليص الجمركي',
        description: 'إنهاء كل الإجراءات الجمركية في وقت قياسي',
        icon: this.icon(
          `<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="m9 15 2 2 4-4"/>`
        ),
      },
      {
        order: '06',
        title: 'التسليم',
        description: 'تسليم في الوقت المحدد أينما كنت',
        icon: this.icon(
          `<path d="M8.25 18.75a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 0 1-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 0 0-3.213-9.193 2.056 2.056 0 0 0-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 0 0-10.026 0 1.106 1.106 0 0 0-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12"/>`
        ),
      },
      {
        order: '07',
        title: 'شراكة مستمرة',
        description: 'دعم مستمر وتطوير علاقة تجارية طويلة الأمد',
        icon: this.icon(
          `<path d="M15.59 14.37a6 6 0 0 1-5.84 7.38v-4.8m5.84-2.58a14.98 14.98 0 0 0 6.16-12.12A14.98 14.98 0 0 0 9.631 8.41m5.96 5.96a14.926 14.926 0 0 1-5.841 2.58m-.119-8.54a6 6 0 0 0-7.381 5.84h4.8m2.581-5.84a14.927 14.927 0 0 0-2.58 5.841m2.699 2.7c-.103.021-.207.041-.311.06a15.09 15.09 0 0 1-2.448-2.448 14.9 14.9 0 0 1 .06-.312m-2.24 2.39a4.493 4.493 0 0 0-1.757 4.306 4.493 4.493 0 0 0 4.306-1.758M16.5 9a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0z"/>`
        ),
        isFinal: true,
      },
    ];
  }

  getNews(): NewsItem[] {
    return [
      {
        title: 'توسع جديد في خطوط الشحن البحري',
        excerpt: 'نعلن عن إضافة خطوط شحن جديدة تربط مصر بأهم الموانئ العالمية.',
        date: 'مارس 2026',
        category: 'أخبار الشركة',
      },
      {
        title: 'شراكة استراتيجية مع علامات عالمية',
        excerpt: 'توقيع اتفاقيات توريد جديدة مع كبرى الشركات العالمية في قطاع الاستهلاك.',
        date: 'فبراير 2026',
        category: 'شراكات',
      },
      {
        title: 'افتتاح مستودع جديد بالقاهرة',
        excerpt: 'مستودع حديث بمساحة 5000 متر مربع لتعزيز خدمات التخزين والتوزيع.',
        date: 'يناير 2026',
        category: 'توسعات',
      },
    ];
  }

  getPageHero(key: string): PageHero {
    const heroes: Record<string, PageHero> = {
      about: {
        tag: 'من نحن',
        title: 'شركة الصفقة للاستيراد والتصدير',
        subtitle:
          'إحدى الشركات الرائدة في الاستيراد والتصدير والشحن والتخليص الجمركي إلى أسواق الشرق الأوسط والعالم',
      },
      services: {
        tag: 'خدماتنا',
        title: 'حلول لوجستية متكاملة',
        subtitle: 'من التوريد إلى التسليم — نغطي كل احتياجاتك',
      },
      products: {
        tag: 'منتجاتنا',
        title: 'جودة عالمية تلبي احتياجاتك',
        subtitle: 'تشكيلة واسعة من المنتجات من أفضل الماركات العالمية',
      },
      'why-us': {
        tag: 'لماذا نحن',
        title: 'لماذا تختار GlobalWay؟',
        subtitle: 'خبرة، ثقة، والتزام بأعلى معايير الجودة',
      },
      news: {
        tag: 'الأخبار',
        title: 'آخر أخبارنا',
        subtitle: 'تابع آخر التطورات والإنجازات',
      },
      contact: {
        tag: 'تواصل معنا',
        title: 'نحن هنا لمساعدتك',
        subtitle: 'تواصل معنا للحصول على أفضل الحلول اللوجستية',
      },
      gallery: {
        tag: 'معرض الصور',
        title: 'معرض أعمالنا',
        subtitle: 'لمحة عن خدماتنا وعملياتنا اللوجستية',
      },
      login: {
        tag: 'تسجيل الدخول',
        title: 'مرحباً بعودتك',
        subtitle: 'سجّل دخولك للوصول إلى حسابك',
      },
    };
    return heroes[key] ?? { tag: '', title: '', subtitle: '' };
  }

  getAboutContent(): AboutPageContent {
    return {
      intro: [
        'شركة الصفقة للاستيراد والتصدير… جودة تثق بها، وشراكة تمتد إلى العالم.',
        'إحدى الشركات الرائدة في مجال الاستيراد والتصدير والشحن والتخليص الجمركي، حيث نلتزم بتوفير منتجات وخدمات عالية الجودة تلبي تطلعات عملائنا في مختلف الأسواق محلياً وإقليمياً وعالمياً.',
      ],
      goal: {
        title: 'الهدف',
        description:
          'نسعى إلى توفير حلول متكاملة للاستيراد والتصدير والشحن والتخليص الجمركي بأعلى معايير الجودة والكفاءة، بما يضمن رضا العملاء وتعزيز الثقة المتبادلة مع شركائنا حول العالم.',
        image:
          'https://images.unsplash.com/photo-1494412574643-ff11b0a5c1c3?auto=format&fit=crop&w=1600&q=80',
      },
      vision: {
        title: 'الرؤية',
        items: [
          'التوسع في أسواق الشرق الأوسط والأسواق الدولية الواعدة',
          'تعزيز مكانة الشركة كجهة موثوقة في الاستيراد والتصدير والشحن',
          'بناء علاقات وشراكات تجارية طويلة الأمد مع العملاء والموزعين',
          'الالتزام بأعلى معايير الجودة والسلامة في جميع العمليات',
          'تقديم أسعار تنافسية وخدمات احترافية تلبي احتياجات العملاء',
          'تحقيق نمو مستدام وزيادة حجم العمليات عامًا بعد عام',
          'مواكبة متطلبات الأسواق العالمية وتطوير الخدمات بما يتناسب مع احتياجات العملاء',
        ],
      },
      stats: [
        { value: 10, prefix: '+', suffix: '', label: 'سنوات خبرة' },
        { value: 50, prefix: '+', suffix: '', label: 'شريك تجاري' },
        { value: 100, prefix: '+', suffix: '', label: 'منتج متنوع' },
        { value: 15, prefix: '+', suffix: '', label: 'سوق عالمي' },
      ],
      phones: ['+20 123 456 7890', '+20 100 000 0000', '+20 111 000 0000'],
    };
  }

  getBrandSection(): {
    subtitle: string;
    paragraphs: string[];
    highlight: string;
    image: string;
    cards: BrandCard[];
  } {
    return {
      subtitle: 'حوّل فكرتك إلى علامة تجارية ناجحة',
      paragraphs: [
        'تمكين الشركات ورواد الأعمال من إنشاء علاماتهم التجارية الخاصة عبر تقديم خدمات التصنيع للغير بجودة متميزة، وأسعار تنافسية، وحلول متكاملة تبدأ من الفكرة وحتى وصول المنتج النهائي إلى السوق.',
      ],
      highlight:
        'نقدم حلولًا متكاملة في التصنيع للغير، والاستيراد والتصدير، لنمنح علامتك التجارية بداية قوية ومستقبلًا واعدًا.',
      image:
        'https://images.unsplash.com/photo-1565793298595-6a879b1d9492?auto=format&fit=crop&w=900&q=70',
      cards: [
        {
          title: 'تطوير المنتج',
          items: ['دراسة الفكرة', 'اختيار التركيبة المناسبة', 'تطوير المنتج بما يتوافق مع احتياجات السوق'],
          icon: this.icon(
            `<path d="M9.75 3.104v5.714a2.25 2.25 0 0 1-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 0 1 4.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3M14.25 3.104c.251.023.501.05.75.082M19.8 15.3l-1.57.393A9.065 9.065 0 0 1 12 15a9.065 9.065 0 0 0-6.23.693L5 14.5m14.8.8 1.402 1.402c1.232 1.232.65 3.318-1.067 3.611A48.309 48.309 0 0 1 12 21c-2.773 0-5.491-.235-8.135-.687-1.718-.293-2.3-2.379-1.067-3.61L5 14.5"/>`
          ),
        },
        {
          title: 'تصميم الهوية والتغليف',
          items: ['تصميم العبوات', 'تصميم اللوجو والهوية', 'تجهيز ملفات الطباعة'],
          icon: this.icon(
            `<path d="M16.862 4.487l1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10"/>`
          ),
        },
        {
          title: 'التصنيع والجودة',
          items: ['تصنيع وفق أعلى معايير الجودة', 'فحص المنتج', 'تعبئة وتغليف احترافي'],
          icon: this.icon(
            `<path d="M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21"/>`
          ),
        },
        {
          title: 'التسليم والدعم',
          items: ['تجهيز المنتج النهائي', 'خدمات الشحن', 'دعم مستمر لتطوير العلامة التجارية'],
          icon: this.icon(
            `<path d="M8.25 18.75a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 0 1-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 0 0-3.213-9.193 2.056 2.056 0 0 0-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 0 0-10.026 0 1.106 1.106 0 0 0-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12"/>`
          ),
        },
      ],
    };
  }

  getGalleryImages(): GalleryImage[] {
    return [
      {
        src: 'https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?auto=format&fit=crop&w=800&q=70',
        alt: 'مستودعات الشحن',
        category: 'لوجستيات',
      },
      {
        src: 'https://images.unsplash.com/photo-1494412574643-ff11b0a5c1c3?auto=format&fit=crop&w=800&q=70',
        alt: 'الشحن الدولي',
        category: 'شحن',
      },
      {
        src: 'https://images.unsplash.com/photo-1553413077-190dd305871c?auto=format&fit=crop&w=800&q=70',
        alt: 'التخزين والتوزيع',
        category: 'مستودعات',
      },
      {
        src: 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?auto=format&fit=crop&w=800&q=70',
        alt: 'منتجات استهلاكية',
        category: 'منتجات',
      },
      {
        src: 'https://images.unsplash.com/photo-1542838132-92c53300491e?auto=format&fit=crop&w=800&q=70',
        alt: 'مواد غذائية',
        category: 'غذاء',
      },
      {
        src: 'https://images.unsplash.com/photo-1565793298595-6a879b1d9492?auto=format&fit=crop&w=800&q=70',
        alt: 'خطوط الإنتاج',
        category: 'تصنيع',
      },
    ];
  }

  getWhyUsValues(): ServiceItem[] {
    return [
      {
        title: 'جودة عالية',
        description: 'نلتزم بأعلى معايير الجودة في كل خدمة نقدمها',
        icon: this.icon(
          `<path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>`
        ),
      },
      {
        title: 'ثقة وموثوقية',
        description: 'شراكة تجارية موثوقة تمتد لسنوات مع عملائنا',
        icon: this.icon(
          `<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>`
        ),
      },
      {
        title: 'وصول عالمي',
        description: 'شبكة واسعة تغطي أكثر من 120 دولة حول العالم',
        icon: this.icon(
          `<circle cx="12" cy="12" r="10"/><path d="M2 12h20"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/>`
        ),
      },
      {
        title: 'دعم متواصل',
        description: 'فريق متخصص لدعمكم على مدار الساعة',
        icon: this.icon(
          `<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>`
        ),
      },
    ];
  }

  getSteps(): StepItem[] {
    return [
      {
        order: '01',
        title: 'تقديم الطلب',
        description: 'تواصل معنا وقدّم تفاصيل احتياجاتك',
        emoji: '📋',
      },
      {
        order: '02',
        title: 'تجهيز الطلب',
        description: 'تجهيز وفحص البضائع وفق معايير الجودة',
        emoji: '📦',
      },
      {
        order: '03',
        title: 'الشحن',
        description: 'شحن آمن وسريع بأفضل وسائل النقل',
        emoji: '🚢',
      },
      {
        order: '04',
        title: 'التخليص الجمركي',
        description: 'إنهاء كل الإجراءات الجمركية في وقت قياسي',
        emoji: '🚚',
      },
      {
        order: '05',
        title: 'التسليم',
        description: 'تسليم في الوقت المحدد أينما كنت',
        emoji: '🤝',
      },
    ];
  }

  getStats(): StatItem[] {
    return [
      {
        value: 98,
        prefix: '+',
        suffix: '%',
        label: 'رضا عملائنا',
        icon: this.icon(
          `<circle cx="12" cy="12" r="10"/><path d="M8 14s1.5 2 4 2 4-2 4-2"/><line x1="9" y1="9" x2="9.01" y2="9"/><line x1="15" y1="9" x2="15.01" y2="9"/>`
        ),
      },
      {
        value: 24,
        prefix: '',
        suffix: '/7',
        label: 'دعم متواصل',
        icon: this.icon(
          `<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>`
        ),
      },
      {
        value: 120,
        prefix: '+',
        suffix: '',
        label: 'دولة حول العالم',
        icon: this.icon(
          `<circle cx="12" cy="12" r="10"/><path d="M2 12h20"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/>`
        ),
      },
      {
        value: 2500,
        prefix: '+',
        suffix: '',
        label: 'شحنة سنويًا',
        icon: this.icon(
          `<path d="M10 17h4V5H2v12h3"/><path d="M20 17h2v-3.34a4 4 0 0 0-1.17-2.83L19 9h-5v8h1"/><circle cx="7.5" cy="17.5" r="2.5"/><circle cx="17.5" cy="17.5" r="2.5"/>`
        ),
      },
    ];
  }

  getProductCategories(): ProductCategory[] {
    return [
      {
        title: 'منتجات استهلاكية',
        subtitle: 'ماركات عالمية',
        image:
          'https://images.unsplash.com/photo-1441986300917-64674bd600d8?auto=format&fit=crop&w=800&q=70',
      },
      {
        title: 'أجهزة إلكترونية',
        subtitle: 'أحدث التقنيات',
        image:
          'https://images.unsplash.com/photo-1498049794561-7780e7231661?auto=format&fit=crop&w=800&q=70',
      },
      {
        title: 'مواد صناعية',
        subtitle: 'جودة موثوقة',
        image:
          'https://images.unsplash.com/photo-1565793298595-6a879b1d9492?auto=format&fit=crop&w=800&q=70',
      },
      {
        title: 'مواد غذائية',
        subtitle: 'طازجة وآمنة',
        image:
          'https://images.unsplash.com/photo-1542838132-92c53300491e?auto=format&fit=crop&w=800&q=70',
      },
      {
        title: 'معدات كهربائية',
        subtitle: 'أساسيات وتوريد',
        image:
          'https://images.unsplash.com/photo-1581092160562-40aa08e78837?auto=format&fit=crop&w=800&q=70',
      },
    ];
  }

  getBrands(): Brand[] {
    return [
      { name: 'SAMSUNG', color: '#1428a0' },
      { name: 'Nestlé', color: '#63513d', font: 'serif' },
      { name: 'Unilever', color: '#1f36c7' },
      { name: 'BOSCH', color: '#ea0016' },
      { name: 'P&G', color: '#003da5', font: 'serif' },
      { name: 'PHILIPS', color: '#0b5ed7' },
      { name: 'LG', color: '#a50034' },
      { name: 'Coca-Cola', color: '#f40009', font: 'cursive' },
    ];
  }
}
